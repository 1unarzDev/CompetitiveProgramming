...
void solve() {
    ...
    for(int i = 0; i < 2; ++i){
        sort(all(r), [&r](pll a, pll b){ return a.F < b.F; });
        if(r[0].F == r[1].F && r[1].F == r[2].F &&
           r[0].S + r[1].S + r[2].S == r[0].F){
            cout << "YES" << endl;
            return;
        }
        if(r[0].F + r[1].F == r[2].F &&
           r[0].S == r[1].S && r[0].S + r[2].S == r[2].F){
            cout << "YES" << endl;
            return;
        }
        if(i == 0){
            for(int j = 0; j < 3; ++j) swap(r[j].F, r[j].S);
        }
    }
