...
void run(ll& n, ll& m, ll& c, vector<vll>& d) {
    ll i, j, ttl;
    i = 1; j = 2;
    vll lp(m);
    vll pr;
...
int32_t main() {
    ...
    ll tt = 3;
    ll n = 1e8;
    ll m = (ll)(n * (log(n) + log(log(n))));
    vector<vll> d(n, vector<int>(tt));
    ...
    ofstream ofs("primes.csv");
    ofs << "Nth Prime,Med,Max,Min" << endl;
    for(ll i = 99; i < n; i += 100) ofs << i + 1 << "," << d[i][tt / 2] << "," << d[i][tt - 1] << "," << d[i][0] << endl;
    ofs.close();
